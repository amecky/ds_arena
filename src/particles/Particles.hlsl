...
PS_Input VS_Main(uint id:SV_VERTEXID) {
	...
	float4 texRect = ParticlesRO[particleIndex].texRect;
	float2 dimension = float2(texRect.z,texRect.w);
	texRect.z += texRect.x;
	texRect.w += texRect.y;
	texRect /= 1024.0;
	float3 position;
	...
	vsOut.color = ParticlesRO[particleIndex].color;
	vsOut.color.a = saturate(1.0 - ParticlesRO[particleIndex].decay * norm);
	return vsOut;
}
