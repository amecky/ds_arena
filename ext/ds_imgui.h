...
namespace gui {
	...
	namespace renderer {
		...
		static void draw_buffer(SpriteBatchBuffer* sprites, UIBuffer* buffer, const p2i& startPos, const p2i& size, const char* fileName) {
			...
			for (uint16_t i = 0; i < num; ++i) {
				...
				fprintf(fp, "pos: %g %g - colors: %g %g %g %g\n", p.x, p.y, buffer->colors[i].r, buffer->colors[i].g, buffer->colors[i].b, buffer->colors[i].a);
			}
			fclose(fp);
			buffer->num = 0;
		}
		...
		void draw_buffer(UIContext* ctx) {
			ctx->sprites->begin();
			draw_buffer(ctx->sprites, ctx->buffer, ctx->startPos, ctx->size, "buffer.txt");
			draw_buffer(ctx->sprites, ctx->overlay_buffer, ctx->startPos, ctx->size, "overlay_buffer.txt");
			ctx->sprites->flush();
		}
	...
	void Message(const char* fmt, ...) {
		p2i pos = _guiCtx->currentPos;
		pos.x -= 10;
		renderer::add_box(_guiCtx->uiContext, pos, 10, 20, ds::Color(51,136,165,255), renderer::ResizeType::RT_X);
		char buffer[1024];
		va_list args;
		va_start(args, fmt);
		vsprintf(buffer, fmt, args);
